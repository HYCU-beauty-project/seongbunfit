...
export function scoreProducts(
  ...
  const scored: ScoredProduct[] = withPricePerMl.map((p) => {
    ...
    const finalScore =
      placement * SCORE_WEIGHTS.placement +
      price * SCORE_WEIGHTS.price +
      budgetSc * SCORE_WEIGHTS.budget;
...
export function getTop3(ingredient: Ingredient, budget: BudgetOption): ScoredProduct[] {
  const candidates = getProductsForIngredient(ingredient.id);
