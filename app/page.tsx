            ...
            {concerns.map((concern, idx) => (
              <AnimateOnScroll key={concern.label} delay={idx * 90} variant="scale">
                ...
                >
                  ...
                  <p className="text-[12.5px] font-semibold text-[var(--color-ink)]">{concern.label}</p>
                </Link>
              </AnimateOnScroll>
            ))}
          </div>
              ...
              <div key={step.title} className="flex items-start">
                <AnimateOnScroll delay={idx * 180} variant="scale">
                  ...
                  >
                    ...
                    <div>
                      <p className="text-[14.5px] font-semibold text-[var(--color-ink)]">{step.title}</p>
                      <p className="mt-1 text-[12px] text-[var(--color-ink-faint)]">{step.desc}</p>
                    </div>
                  </Link>
                </AnimateOnScroll>
                {idx < steps.length - 1 && (
          ...
          <AnimateOnScroll className="md:sticky md:top-28">
            <div>
              ...
              <ul className="mt-5 space-y-2">
                {goodPoints.map((p) => (
                  <li key={p} className="flex items-center gap-1.5 text-[12.5px] text-[var(--color-ink-soft)]">
          ...
          <div className="mx-auto flex max-w-5xl flex-col items-center gap-6 px-6 text-center md:flex-row md:text-left">
            <div className="flex-1">
              ...
              <p className="mt-2 text-[13px] text-[var(--color-ink-soft)] leading-relaxed">
                준비 중인 얼굴 피부 진단(안면인식) 기능이에요. 촬영 환경에 따라 결과가 달라질 수
                있어 참고용으로만 활용해주시고, 모바일에서는 카메라 권한이 필요해요.
              </p>
              <Link
                href="/face-analysis"
            ...
            >
              ...
              <div className="relative flex h-40 w-40 items-center justify-center rounded-full bg-[var(--color-primary-soft)] transition-transform group-hover:scale-105">
                <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--color-primary)]">
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            {[
              ...
              },
              {
                badge: "다음",
                title: "더 정확하게, 더 가깝게",
                items: ["얼굴 피부 진단(안면인식)", "화장품 카테고리 확장(크림·토너 등)", "주요 이커머스 플랫폼 구매 연동 API 도입", "오픈 기념 이벤트"],
              },
              {
                badge: "이후",
                title: "함께 성장하기",
                ...
                <div className="h-full rounded-2xl bg-white p-6 shadow-sm">
                  ...
                  <ul className="mt-3 space-y-2">
                    {phase.items.map((item) => (
                      <li key={item} className="flex items-start gap-1.5 text-[12.5px] text-[var(--color-ink-soft)]">
