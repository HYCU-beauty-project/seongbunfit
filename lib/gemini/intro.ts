...
/**
 * 사용자가 실제로 쓴 표현 반영한 자연스러운 인트로 문장을 AI가 생성함.
 * 예: "피부가 따가워요" → "따가움이 느껴지시는군요! 자극 완화에 도움이 될 성분을 보여드릴게요."
 * 키 없거나 실패하면 category.intro 고정 템플릿으로 폴백
 */
export async function generateIntro(userText: string, category: Category): Promise<string> {
  ...
  for (const model of [MODEL, FALLBACK_MODEL]) {
    try {
      ...
      // finishReason이 MAX_TOKENS면 문장이 중간에 잘렸다는 뜻이라
      // 어색하게 끊긴 문장 그대로 보여주는 대신 안전하게 템플릿으로 폴백
